...
def gimmelists():
    ...
    for result in results:
        ...
        for n in course_nums:
            if courseware_num_to_name.get(n) is not None:
                course_name = courseware_num_to_name.get(n)[0]
                break
        # print course_name;
        if course_name == '':
            continue
        elif result.get('prerequisites') is None:
            ...
            if len(ors) > 0:
                for o in ors:
                    ...
                    for course in courses:
                        if 'permission of instructor' not in course and 'Permission of Instructior' not in course and 'None' not in course and course != '':
                            if groups.get(course) is not None:
    ...
    for course in course_dictionary.keys():
        ...
        for rec in pre_rex:
            ...
            for r in rec:
                # print r.strip()
                if courseware_num_to_name.get(r.strip()) is not None:
